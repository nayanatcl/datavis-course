function Network() {
  ...
  function setupData(data) {
    console.log(degreeLinks(data));
    var circleRadius, countExtent;
    var getDegree = degreeLinks(data);
    // initialize circle radius scale
    countExtent = d3.extent(data.nodes, function(d) {
      return getDegree[d.id]; 
    });

    circleRadius = d3.scale.linear().range([3, 20]).domain(countExtent);
    ...
    // add radius to the node so we can use it later
      n.degree = getDegree[n.id];
      n.radius = circleRadius(getDegree[n.id]);
      //n.radius = 10 + (n.value * 2);
    });
  ...
  function updateNodes() {
    ...
    .append("circle").attr("class", "node").attr("cx", function(d) {
      return d.x;})
    .style("fill", function (d) { return '#FF6347'; })
    .attr("cy", function(d) {
      return d.y;})
  ...
  //node.append("circle")
  //     .attr("r", function(d) {return d3.sum(force.links().filter(function(p) {return p.source == d || p.target == d}), function(p) {return p.weight})})
